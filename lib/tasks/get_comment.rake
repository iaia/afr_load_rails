...
namespace :get_comment do
  ...
  class CommentGetter
    def initialize(now, tv_info_name, provider_name)
      ...
      prepare_twitter(tv_info_name) if provider_name == "Twitter"
    end
    ...
    def get
      EM.run do
        ...
        puts "#{@tv_program_info.name}:" \
             " #{@tv.title}(on_air_time:#{@on_air_time}): #{@topics}"
        ...
        client.track(@topics) do |status|
          ...
          begin
            Comment.create(provider_id: @provider.id,
                           comment_id_on_provider: status.id) do |comment|
